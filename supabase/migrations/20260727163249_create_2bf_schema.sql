...
CREATE POLICY "admin_insert_profile" ON public.profiles FOR INSERT TO authenticated WITH CHECK (public.current_user_is_admin());
...
CREATE POLICY "admin_update_profiles" ON public.profiles FOR UPDATE TO authenticated USING (public.current_user_is_admin()) WITH CHECK (public.current_user_is_admin());
...
CREATE POLICY "admin_write_warehouses" ON public.warehouses FOR ALL TO authenticated USING (public.current_user_is_admin()) WITH CHECK (public.current_user_is_admin());
...
CREATE POLICY "admin_write_products" ON public.products FOR ALL TO authenticated USING (public.current_user_is_admin()) WITH CHECK (public.current_user_is_admin());
...
CREATE POLICY "write_stock_movements" ON public.stock_movements FOR INSERT TO authenticated WITH CHECK (true);

-- barcode_history policies
...
CREATE POLICY "write_barcode_history" ON public.barcode_history FOR INSERT TO authenticated WITH CHECK (true);
...
CREATE POLICY "write_audit_logs" ON public.audit_logs FOR INSERT TO authenticated WITH CHECK (true);
