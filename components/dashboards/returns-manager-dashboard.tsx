...
import {
  RotateCcw, AlertTriangle, CheckCircle, FileWarning, Settings,
...
export function ReturnsManagerDashboard() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const result = await fetchDashboardData(filters, 'returns_manager');
      setData(result);
    } catch (e: any) {
  ...
  return (
    <DashboardShell>
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Returns Manager Dashboard</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Welcome back, {profile.full_name || 'Returns Manager'}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button variant="outline" onClick={() => { if (data) exportDashboardToExcel(data, filters, profile?.full_name || 'User', 'Returns Manager'); }} className="h-10 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
          ...
          <div className="flex items-center gap-3 ml-2 pl-4 border-l border-slate-200 dark:border-slate-800">
            ...
            <div className="hidden sm:block">
              <p className="text-sm font-bold text-slate-900 dark:text-white leading-tight">{profile.full_name || 'Returns Manager'}</p>
            ...
            <Card className="shadow-sm border-slate-200 flex flex-col">
              ...
              <CardContent className="p-4 grid grid-cols-1 gap-3 flex-1 content-start">
                <Link href="/returns" className="flex items-center gap-2 p-3 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors">
                  ...
                  <span className="text-xs font-medium text-slate-700">Approve / Reject Returns</span>
                </Link>
                <Link href="/reports" className="flex items-center gap-2 p-3 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors">
                  ...
                  <span className="text-xs font-medium text-slate-700">Returns Reports</span>
                </Link>
              </CardContent>
            </Card>
          </div>
            ...
            <div className="flex items-center justify-between p-3 rounded-lg border border-amber-200 bg-amber-50">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-sm font-semibold text-amber-900 truncate">
                   Review pending return approvals
                </span>
              </div>
              <Link href="/returns" className="text-xs font-semibold px-3 py-1.5 rounded bg-amber-100 text-amber-800 hover:bg-amber-200 transition-colors whitespace-nowrap ml-2 shrink-0">Review Now</Link>
            </div>
