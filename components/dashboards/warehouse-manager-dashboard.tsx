...
import {
  Package, ArrowDownToLine, ArrowUpFromLine, AlertTriangle, Settings,
...
export function WarehouseManagerDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const result = await fetchDashboardData(filters, 'warehouse_manager', warehouseId);
      setData(result);
    } catch (e: any) {
  ...
  const totalOut = data?.warehouseOverview.reduce((s, w) => s + w.dispatchedToday, 0) ?? 0;
  ...
  return (
    <DashboardShell>
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Warehouse Manager Dashboard</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Welcome back, {profile.full_name || 'Warehouse Manager'}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button variant="outline" onClick={() => { if (data) exportDashboardToExcel(data, filters, profile?.full_name || 'User', 'Warehouse Manager'); }} className="h-10 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
          ...
          <div className="flex items-center gap-3 ml-2 pl-4 border-l border-slate-200 dark:border-slate-800">
            ...
            <div className="hidden sm:block">
              <p className="text-sm font-bold text-slate-900 dark:text-white leading-tight">{profile.full_name || 'Warehouse Manager'}</p>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <Card className="shadow-sm border-slate-200 flex flex-col">
              ...
              <CardContent className="p-4 grid grid-cols-1 gap-3 flex-1 content-start">
                <Link href="/warehouse/corrections" className="flex items-center gap-2 p-3 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors">
                  <AlertTriangle className="w-4 h-4 text-rose-500 shrink-0" />
                  <span className="text-xs font-medium text-slate-700">Review Corrections</span>
                </Link>
                <Link href="/warehouse/transfers" className="flex items-center gap-2 p-3 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors">
                  ...
                  <span className="text-xs font-medium text-slate-700">Manage Transfers</span>
                </Link>
                <Link href="/inventory" className="flex items-center gap-2 p-3 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors">
                  ...
                  <span className="text-xs font-medium text-slate-700">Inventory Management</span>
                </Link>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="flex items-center justify-between p-3 rounded-lg border border-amber-200 bg-amber-50">
              ...
              <Link href="/inventory" className="text-xs font-semibold px-3 py-1.5 rounded bg-amber-100 text-amber-800 hover:bg-amber-200 transition-colors whitespace-nowrap ml-2 shrink-0">View Inventory</Link>
            </div>
            ...
            <div className="flex items-center justify-between p-3 rounded-lg border border-blue-200 bg-blue-50">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-sm font-semibold text-blue-900 truncate">
                   Review pending inventory corrections
                </span>
              </div>
              <Link href="/warehouse/corrections" className="text-xs font-semibold px-3 py-1.5 rounded bg-blue-100 text-blue-800 hover:bg-blue-200 transition-colors whitespace-nowrap ml-2 shrink-0">Review Now</Link>
            </div>
          </div>
